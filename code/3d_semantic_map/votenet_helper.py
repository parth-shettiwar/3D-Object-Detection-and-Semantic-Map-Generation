...
def votenet_build(flag=0):
    ...
    if DATASET == 'sunrgbd':
        ...
        if flag==0:
            ...
            from sunrgbd_detection_dataset_1 import DC
    ...
    if flag==0:
        ...
        type2class={'book':0, 'bottle':1, 'bowl':2, 'cup':3, 'keyboard':4, 'mouse':5, 'plant':6, 'tv':7}
...
def votenet_forward(net, point_cloud):
    ...
    try:
        ...
        print('Finished detection. %d objects detected.' %
              (len(pred_map_cls[0])))
        return pred_map_cls
    except:
...
def votenet_detection(net, observations, flag=0):
    ...
    if detections[0]:
        ...
        for detection in detections:
            ...
            if flag==0 and detection[0] > net.type2class['desk']:
                continue
            # Bottle giving bad issues of false positives
            # elif flag==1 and detection[0] == net.type2class['bottle']:
            #     continue
            result = {
...
def votenet_nms(all_results, class_list):
    nms_iou_cls = 0.1 #0.1 # this threshold for removing duplicates that are of the same class
    boxes_world = []
    results_list = []
    ...
    nms_iou = 0.25 #0.25  # this threshold is used to remove duplicates that are not of the same class
    boxes_world = []
    results_list = []
