...
class GroupFreeDetector(nn.Module):
    ...
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr,
        ...
        if self.sampling == 'fps':
            ...
            self.points_obj_cls = PointsObjClsModule(288).to(device)
            self.gsample_module = GeneralSamplingModule()
        else:
            raise NotImplementedError
        # Proposal
        if self.size_cls_agnostic:
            self.proposal_head = ClsAgnosticPredictHead(num_class, num_heading_bin, num_proposal, 288).to(device)
        else:
            self.proposal_head = PredictHead(num_class, num_heading_bin, num_size_cluster,
                                             mean_size_arr, num_proposal, 288).to(device)
        if self.num_decoder_layers <= 0:
            # stop building if has no decoder layer
            return

        # Transformer Decoder Projection
        self.decoder_key_proj = nn.Conv1d(288, 288, kernel_size=1).to(device)
        self.decoder_query_proj = nn.Conv1d(288, 288, kernel_size=1).to(device)
    ...
    def forward(self, inputs):
        ...
        """
        end_points = {}
        # print(inputs['point_clouds'].shape,"Inside backbone net pc shape")
        ...
        # print("KEEYYY",end_points.keys())
        # for key in end_points.keys():
        #   if('size_scores' in key):
        #     print(key)
        return end_points
